<script lang="ts">
	import type { RecursiveJSONSchema } from 'src/lib/app/api/.generated/api';
...
{#if data}
	...
	{:else if data.type === 'array' && data.items}
		...
		<code>{name}: <span class:required class={typeColors[data.type]}>{data.type}</span> [</code>
		<br />
	
	</pre>

		<div class="my-1 ml-12 border-b border-l border-dashed border-gray-300 py-2 pl-0">
	...
	<code class:required
				>{name}: <span class={typeColors[data.type]}>{data.const ? data.const : data.type}</span
				></code
			>
</pre>
	{/if}
