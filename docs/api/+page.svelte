<script lang="ts">
	...
	import { Svetch } from 'src/lib/app/api/.generated/client';
	...
	async function testAPI(method: string, endpoint: string) {
		...
		try {
			const { data, error, statusText } = await Svetch.request(endpoint, method, {
				// type: ignore
			...
			error_cause = JSON.parse(error.cause).message ? JSON.parse(error.cause).message : error.cause;
...
{#await loadSchema()}
	...
	<div transition:fly class="flex h-screen w-full place-content-center place-items-center">
					...
					{#each Object.entries(schema.properties) as [path, methods]}
						{#if methods && methods.properties}
							<li>
								<details open>
									<summary
										><a class="inline-flex place-items-center space-x-2" href="#{path}"
									...
									<ul>
										{#each Object.keys(methods.properties) as method (method)}
											<li class="{text_colors[method]} font-bold">
												<a href="#{path}-{method}"><Icon icon={icons[method]} />{method}</a>
											</li>
										{/each}
									</ul>
								</details>
				...
				<div class="navbar sticky top-0 z-10 w-full bg-base-300">
					...
					<div class="hidden flex-none lg:block">
						<ul class="menu menu-horizontal">
							<button class="btn-ghost btn" on:click={() => table_of_contents.showModal()}
									...
									>
										...
										{#each path.split('/') as path_part}
											...
											{#if path_part.includes(':')}
												<div class="badge badge-lg rounded-md bg-gray-500 text-white">
													{path_part.replace(':', '')}
												</div>
											{:else}
											...
											<div class="md:ml-2">
												<Collapsible>
													<svelte:fragment slot="title">
														<div class="inline-flex place-items-center space-x-2">
															<div
																id="{path}-{method}"
																class="badge {colors[method]} badge-lg gap-2 text-white"
															>
													...
													<div class="flex flex-col space-y-2">
														{#if typeof method_data === 'object' && method_data.properties && method_data.properties.parameters}
															{@const parameters = method_data.properties.parameters.properties}
															{@const responses = method_data.properties.responses}
															{@const errors = method_data.properties.errors}
															{@const testing =
																path_being_tested == path && method_being_tested == method}
															{#if parameters}
																<div class="card-title text-xl font-medium">
																	<Icon class="inline" inline icon="carbon:parameter" />
																...
																{#if path_being_tested == path && method_being_tested == method && parameters.path && parameters.path.properties}
																	...
																	<div class="contents" transition:slide>
																		<h4 class="text-sm font-normal">
																			<Icon class="inline" inline icon="carbon:folders" /> Path
																		</h4>
																		<div class="m-2 flex space-x-2">
																			{#each Object.entries(parameters.path.properties) as [param, type]}
																				...
																				<input
																					...
																					name={param}
																					id={param}
																					bind:value={testing_params.path[param]}
																				/>
																			{/each}
																		</div>
																	</div>
																{/if}
																<div class="rounded-lg border border-gray-300 p-2 shadow-inner">
															...
															{#if testing && parameters.body && parameters.body.properties}
																<div class="form-control">
																	...
																	<textarea
																		rows={Object.keys(testing_params.body).length + 2}
																		class="textarea-bordered textarea-primary textarea h-auto"
																		name="body"
																		id="body"
																		value={JSON.stringify(testing_params.body, null, '\t')}
																		on:input={(e) => {
																			try {
																				testing_params.body = JSON.parse(e.target.value);
																			} catch (e) {
																				console.error(e);
																			}
																		}}
															...
															{#if path_being_tested === path && method_being_tested === method}
																<div class="" transition:slide>
																	{#if error_response}
																		...
																		>
																			<Icon icon="carbon:error" class="text-error" />
																			<div class="mockup-code" transition:slide={{ duration: 200 }}>
																				<pre class="text-error"><code
																						>{JSON.stringify(error_response, null, 2)}</code
																					></pre>
																			</div>
																		</span>
																		{#if error_cause}
																			<div class="mockup-code" transition:slide={{ duration: 200 }}>
																				<pre class="text-error"><code
																						>{JSON.stringify(error_cause, null, 2)}</code
																					></pre>
																			</div>
																		{/if}
																	{:else if testing_response}
																		<span
																			...
																			out:fly={{ x: -25, duration: 200 }}
																		>
																			<Icon icon="carbon:checkmark" class="text-success" />
																			Success!
																		</span>
																		<div
																		...
																		>
																			<pre class="whitespace-pre-wrap break-words"><code
																					>{JSON.stringify(testing_response, null, '\t')}</code
																				></pre>
																		</div>
																	{:else if working}
																		<span
																			...
																			out:fly={{ x: -25, duration: 200 }}
																		>
																			<Icon icon="carbon:circle-dash" class="animate-spin" />
																			<span>Testing...</span>
																		</span>
																	{/if}
																</div>
															...
															<div class="card-actions justify-end">
																<button
																	...
																	on:click={() => {
																		...
																		if (parameters?.body && parameters?.body.properties) {
																			testing_params.body = Object.entries(
																				...
																				if (value.type == 'number') {
																					...
																					} else if (value.format && value.format == 'date') {
																						acc[key] = new Date().toISOString();
																					} else {
																						acc[key] = '';
																					}
																		...
																		const firstInput = document.getElementById(`${path}-${method}`);
																		if (firstInput) {
																...
																<button
																	...
																	on:click={() => testAPI(method, path)}>Test</button
																>
															</div>
														{/if}
													</div>
